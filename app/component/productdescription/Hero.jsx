...
const product = {
  name:
    'Apple - MacBook Air 15" Laptop - M2 chip - 16GB Memory - 1TB SSD (Latest Model) - Midnight',
      ...
      <Flex direction={{ xs: "column", lg: "row" }} gap={24}>
        <Box>
          ...
          <Flex gap={12} direction={"column"} align={"center"}>
            <Image
              w={{ xs: 300, md: "80%", lg: 380 }}
              src={selectedImage}
              alt={product.name}
            />
            <Flex
              className="mob-scroll"
              w={{ xs: 320, md: 550, lg: 350 }}
              gap={12}
        ...
        <Box>
          <Text display={{ xs: "none", md: "block" }} fz={24} fw={600}>
            {product.name}
          </Text>
          <Flex
            gap={12}
            direction={{ xs: "column", md: "row", lg: "column" }}
            align={{ xs: "start", md: "center", lg: "start" }}
          >
          ...
          <Group spacing="xs" mt="xs">
            {product.sizes.map((size, index) => (
              <Button
                key={index}
                variant={selectedSize === size.size ? "filled" : "outline"}
